...
def msgBytesOnWire(parsedStats, xmlParsedDic, msgBytesOnWireDigest):
    ...
    for size in sizes:
        ...
        for id in receiverHostIds:
            # print("id: ", id)
            bytesStatsKey = 'host[{0}].trafficGeneratorApp[0].msg{1}BytesOnWire:histogram'.format(id, size)
            bytesOnWire.cnt += int(hosts.access(bytesStatsKey + '.count'))
            totalCnt +=  int(hosts.access(bytesStatsKey + '.count'))
            bytesOnWire.bytes += (hosts.access(bytesStatsKey + '.sum'))
            totalBytes += (hosts.access(bytesStatsKey + '.sum'))
...
def e2eStretchAndDelay(parsedStats, xmlParsedDic, msgBytesOnWireDigest, e2eStretchAndDelayDigest):
    ...
    for size in sizes:
        ...
        e2eStretchList = list()
        for id in receiverHostIds:
            e2eDelayHistogramKey = 'host[{0}].trafficGeneratorApp[0].msgE2EDelay:histogram.bins'.format(id)
            e2eDelayStatsKey = 'host[{0}].trafficGeneratorApp[0].msgE2EDelay:stats'.format(id)
            queuingDelayHistogramKey = 'host[{0}].trafficGeneratorApp[0].msgQueuingDelay:histogram.bins'.format(id)
            queuingDelayStatsKey = 'host[{0}].trafficGeneratorApp[0].msgQueuingDelay:stats'.format(id)
            e2eStretchHistogramKey = 'host[{0}].trafficGeneratorApp[0].msgE2EStretch:histogram.bins'.format(id)
            e2eStretchStatsKey = 'host[{0}].trafficGeneratorApp[0].msgE2EStretch:stats'.format(id)
...
def main():
    ...
    # print subprocess.Popen('cd {0}; Rscript PlotQueueWaitTime.r'.format(plotPath), shell=True, stdout=subprocess.PIPE).stdout.read()
