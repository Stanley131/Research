...
class AttrDict(dict):
	...
	#print names
		...
		"""
                name = names[-1]
                for key, subContainer in container.items():          
                    if name in subContainer: 
                        #print(name, subContainer[name])        
                        return subContainer[name]
            
                if "stats" in name: 
                    name = name.replace("stats", "histogram")
                else: 
                    name = name.replace("histogram", "stats")
