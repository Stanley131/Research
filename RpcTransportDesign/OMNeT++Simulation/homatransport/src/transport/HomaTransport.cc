...
HomaTransport::ReceiveScheduler::processReceivedPkt(HomaPkt* rxPkt)
{
    ...
    *sumGrantsInGap = 0;
    ...
    old.setVar(schedSenders->numToGrant, schedSenders->headIdx,
        schedSenders->numSenders, s, 0, rxPkt->getMsgId());
    ...
    cur.setVar(schedSenders->numToGrant, std::get<1>(ret), std::get<2>(ret),
        s, std::get<0>(ret), rxPkt->getMsgId());
...
HomaTransport::ReceiveScheduler::SchedSenders::handleBwUtilTimerEvent(
    ...
    old.setVar(numToGrant, headIdx, numSenders, lowPrioSx, indToGrant, 0);
	...
	switch (m_profile) {
		case 1: // W1 (alpha = 10)
			if (m_nonBlind) { 
				...
				limits = {2.2069E+00, 1.7569E+00, 1.3986E+00, 1.1134E+00, 8.8634E-01, 7.0560E-01, 5.6171E-01};
			} else {
		...
		case 2: // W2 (alpha = 10)
			if (m_nonBlind) {
				...
				limits = {5.7174E+00, 3.7057E+00, 2.4019E+00, 1.5568E+00, 1.0090E+00, 6.5399E-01, 4.2388E-01};
			} else {
		...
		case 3: // W3 (alpha = 10)
			if (m_nonBlind) {
				...
				limits = {1.5417E+01, 6.5709E+00, 2.8007E+00, 1.1937E+00, 5.0878E-01, 2.1685E-01, 9.2426E-02};	
		...
		case 4: // W4 (alpha = 10)
			if (m_nonBlind) {
				...
				limits = {1.4803E+00, 8.9715E-01, 5.4375E-01, 3.2955E-01, 1.9973E-01, 1.2105E-01, 7.3368E-02};
			} else {
		...
		case 5: // W5 (alpha = 10)
			if (m_nonBlind) {
				...
				limits = {1.6774E+00, 9.7827E-01, 5.7053E-01, 3.3273E-01, 1.9405E-01, 1.1317E-01, 6.6000E-02};
			} else {
